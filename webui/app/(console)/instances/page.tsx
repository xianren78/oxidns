...
export default function InstancesPage() {
  ...
  const [states, setStates] = useState<Record<string, InstanceHealthState>>({});
  ...
  const refresh = useCallback(async () => {
    ...
    setStates(
      Object.fromEntries(
        endpoints.map((endpoint) => [endpoint.id, { status: "loading" }]),
      ),
    );
    await refreshEndpointHealth(endpoints, (endpointId, state) => {
      ...
      <main className="oxidns-dialog-scrollbar min-h-0 flex-1 overflow-auto p-4 sm:p-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {endpoints.map((endpoint) => {
            ...
            const health = state?.status === "online" ? state.health : null;
            return (
