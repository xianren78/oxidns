...
/** @deprecated Use endpoints and activeEndpointId for new code. */
export type ServerConfig = Omit<Endpoint, "id" | "name">;
...
function endpointServerConfig(endpoint: Endpoint): ServerConfig {
...
export interface AuthState {
  ...
  /** @deprecated Compatibility alias for older consumers and tests. */
...
export const useAuthStore = create<AuthState>()(
  persist(
    (set, get) => ({
      ...
      serverConfig: endpointServerConfig(defaultEndpoint),
      ...
      updateEndpoint: (id, endpoint) =>
        set((state) => ({
          ...
          ...(id === state.activeEndpointId
            ? {
                serverConfig: endpointServerConfig({ ...endpoint, id }),
                isConnected: false,
                isAuthenticated: false,
              }
            : {}),
      ...
      deleteEndpoint: (id) =>
        set((state) => {
          ...
          return {
            ...
            ...(id === state.activeEndpointId
              ? {
                  serverConfig: endpointServerConfig(next),
                  isConnected: false,
                  isAuthenticated: false,
      ...
      switchEndpoint: async (id) => {
        const endpoint = get().endpoints.find((endpoint) => endpoint.id === id);
        if (!endpoint) return false;
        set((state) => ({
          activeEndpointId: id,
          serverConfig: endpointServerConfig(endpoint),
          isConnected: false,
          isAuthenticated: false,
          isConnecting: true,
          connectionError: null,
      ...
      logout: () =>
        set((state) => ({
          ...
          serverConfig: { ...state.serverConfig, password: "" },
        })),
    ...
    }),
    {
      ...
      migrate: (persisted: unknown) => {
        ...
        if (old.endpoints?.length) {
          const endpoint =
            old.endpoints.find((item) => item.id === old.activeEndpointId) ??
            old.endpoints[0];
          return {
            ...old,
            activeEndpointId: endpoint.id,
        ...
        const serverConfig =
          old.serverConfig ?? endpointServerConfig(defaultEndpoint);
        return {
          ...old,
          serverConfig,
          endpoints: [{ ...defaultEndpoint, ...serverConfig }],
          activeEndpointId: defaultEndpoint.id,
        } as AuthState;
      },
      partialize: (state) => ({
